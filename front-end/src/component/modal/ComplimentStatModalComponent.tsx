...
export default function ComplimentStatModalComponent({open,close,header,modalSize}) {
    ...
    function getData(){
        ...
        if(header.selChk!==null){
            procGetAxios("/user/service/request/tyCdRegist?selectDate="+header.selDt+"&selectType="+header.selTy+"&items="+header.selChk,state.token,"application.json",setDetail);
        }
            ...
            {open?(
                <section className={modalSize}>
                    ...
                    <div className="m-3 mb-8">
                        <h5 className="text-black">{date[0]}년 {Number(date[1])}월 {Number(date[2])}일</h5>
                        <div className="bg-light p-3 small">
                                <div className="pb-1">유형 : {type} </div>
