...
/**
 * @Project     : HelpDesk
 * @FileName    : AttachServiceComponent.tsx
 * @Date        : 2022-05-16
 * @author      : 김지인
 * @description : 서비스요청 첨부파일 변경 & 다운로드 컴포넌트
 */

export default function AttachServiceComponent({rqstId, edit}) {
...
function Handler(filename) {
 ...
 function delectFile(attachId){
  let delectAttach;
  procPostAxios('/user/service/request/attach/'+attachId+'/delete', state.token, contentType, delectAttach, test, error )   
 }
  ...
  if(extSecurity.includes(ext)) {
    ...
    if(attach.length < 5){
      if(e.target.files[0] != null){
                ...
                procPostAxios("/user/service/request/"+rqstId+"/attach", state.token, contentType, formData, updateData, error)               
          }
      } else{
